"""
Source: https://huggingface.co/datasets/nvidia/PhysicalAI-Spatial-Intelligence-Warehouse/blob/main/utils/compute_scores.py
"""
...
def main():
    parser = argparse.ArgumentParser(
        description="Compute scores from normalized answers"
    )
    parser.add_argument(
        "--gt_path", help="Path to the ground truth normalized answers file"
    )
    parser.add_argument(
        "--pred_path", help="Path to the prediction normalized answers file"
    )
    ...
    for gt_item in gt_data:
        ...
        if question_type == "quantitative":
            ...
            final_metrics[category].update(
                compute_errors(np.array([gt_value])[None], np.array([pred_value])[None])
            )
    ...
    for qual_cat in qualitative_dict.keys():
        ...
        result_dict[f"Qual_{qual_cat}_acc"] = (
            np.sum(qualitative_dict[qual_cat])
            / (len(qualitative_dict[qual_cat]) + EPSILON)
            * 100
        )

    result_dict[f"Qual_overall_acc"] = (
        correct_qualitative / (total_qualitative + EPSILON) * 100
    )
    ...
    for quant_cat in available_quant_cats:
        ...
        result_dict[f"Quan_{quant_cat}_acc"] = (
            ...
            / (len(quantitative_success_dict[quant_cat]) + EPSILON)
            * 100
        )
        ...
        if quant_cat in quantitative_error_dict:
            ...
            result_dict[f"Quan_{quant_cat}_err"] = (
                ...
                / (len(quantitative_error_dict[quant_cat]) + EPSILON)
                * 100
            )
    ...
    for category in final_metrics:
        ...
        if metrics_value:
            ...
            result_dict[f"Quan_{category}_absrel"] = formatted_metrics.get(
                "abs_rel", "N/A"
            )
    ...
    for category in sorted(quantitative_success_dict.keys()):
        ...
        if count > 0:
            ...
            print(
                f"{category.capitalize()} ({count}): {correct_count}/{count} = {accuracy:.2f}%"
            )
            ...
            if (
                category in quantitative_error_dict
                and len(quantitative_error_dict[category]) > 0
            ):
                err_rate = (
                    np.sum(quantitative_error_dict[category])
                    / len(quantitative_error_dict[category])
                    * 100
                )
    ...
    for category in ["count", "distance"]:
        if (
            category in quantitative_success_dict
            and len(quantitative_success_dict[category]) > 0
        ):
            correct_count = sum(quantitative_success_dict[category])
            ...
            print(
                f"{category.capitalize()} (weighted): {accuracy:.2f}% * {category_weights[category]:.2f} = {weighted_scores[category]:.2f}"
            )
    ...
    for category in ["left_right", "mcq"]:
        if category in qualitative_dict and len(qualitative_dict[category]) > 0:
            ...
            total_weight += category_weights[category]
            print(
                f"{category} (weighted): {accuracy:.2f}% * {category_weights[category]:.2f} = {weighted_scores[category]:.2f}"
            )
    ...
    if total_quantitative > 0:
        ...
        print(
            f"Quantitative: {correct_quantitative}/{total_quantitative} = {quan_overall_acc:.2f}%"
        )
    ...
    if total_qualitative > 0:
        ...
        print(
            f"Qualitative: {correct_qualitative}/{total_qualitative} = {qual_overall_acc:.2f}%"
        )
    ...
    if (total_quantitative + total_qualitative) > 0:
        overall_acc = (
            (correct_quantitative + correct_qualitative)
            / (total_quantitative + total_qualitative)
            * 100
        )
        print(
            f"Overall: {correct_quantitative + correct_qualitative}/{total_quantitative + total_qualitative} = {overall_acc:.2f}%"
        )
    ...
    try:
        ...
        full_results = {"evaluation": result_dict_converted, "entries": []}
        ...
        for gt_item in gt_data:
            ...
            entry = {
                ...
                "prediction": {"normalized_answer": pred_lookup[question_id]},
            }
        ...
        full_results_path = os.path.join(
            predictions_dir, f"full_results_{timestamp}.json"
        )
